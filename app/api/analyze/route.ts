...
{"strategies": [{"trend": "출제 경향", "strategy": "대응 학습 전략"}]}`,
    ...
    const rotate = <T,>(arr: T[], n: number) => arr.length === 0 ? arr : [...arr.slice(n % arr.length), ...arr.slice(0, n % arr.length)]

    const settled = await Promise.allSettled(
      stagePrompts.map(async (stage, idx) => {
        if (idx > 0) await new Promise((r) => setTimeout(r, idx * 250))
        const stageKeyOrder = rotate(aliveFirst, idx)
        const res = await runStage(imageParts, stage.prompt, stageKeyOrder)
        return { key: stage.key, ...res }
      })
    )
